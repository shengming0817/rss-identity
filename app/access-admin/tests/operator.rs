...
use access_core::account::Password;
use access_postgres::*;
...
async fn maintenance_file_and_settlement() -> anyhow::Result<()> {
    ...
    let dir = std::env::temp_dir().join(format!("access-maintenance-{}", uuid::Uuid::new_v4()));
    ...
    for initialize in [true, false] {
        ...
        for fault in [None, Some(PgTransactionFault::RollbackFailedAfterAck)] {
            ...
            let initialized: bool = sqlx::query_scalar(
                "SELECT bootstrap_tenant IS NOT NULL FROM access_authority.deployment",
            )
            .fetch_one(&f.owner)
            .await?;
            assert_eq!(initialized, !initialize);
            let epochs: Vec<i64> =
                sqlx::query_scalar("SELECT auth_epoch FROM access_authority.accounts")
                    .fetch_all(&f.owner)
                    .await?;
        ...
        let epoch: i64 = sqlx::query_scalar("SELECT auth_epoch FROM access_authority.accounts")
            .fetch_one(&f.owner)
            .await?;
