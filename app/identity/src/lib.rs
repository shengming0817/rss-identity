...
pub enum AppError {
    #[error("application lifecycle did not finish cleanly")]
    Shutdown,
    #[error("required provider unavailable or misconfigured")]
    Provider,
    #[error("installation refused or result unknown; inspect installation before retrying")]
    Migration,
...
pub fn read_secret(path: &Path) -> Result<Zeroizing<String>, AppError> {
    let file = OpenOptions::new()
        .read(true)
        .custom_flags(libc::O_NOFOLLOW | libc::O_NONBLOCK)
        .open(path)
        .map_err(|_| AppError::File)?;
    let meta = file.metadata().map_err(|_| AppError::File)?;
...
pub fn read_public_file(path: &Path, limit: usize) -> Result<Vec<u8>, AppError> {
    if limit > 1024 * 1024 {
    ...
    let file = OpenOptions::new()
        ...
        .open(path)
        .map_err(|_| AppError::File)?;
    let metadata = file.metadata().map_err(|_| AppError::File)?;
