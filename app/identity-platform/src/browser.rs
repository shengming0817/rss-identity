...
async fn login_with_opener(
    ...
    let mut child = Command::new(opener)
        ...
        .spawn()
        .map_err(|_| Error::Unavailable)?;
    let wait = receive_code(listener, &authorize, &state, &mut child);
...
mod tests {
    ...
    #[tokio::test]
    async fn invalid_loopback_requests_do_not_complete_or_exchange_the_code() {
        ...
        let wait = tokio::spawn(async move {
            ...
            super::receive_code(
                listener,
                &url::Url::parse("https://identity.test/authorize").unwrap(),
                &state_for_server,
                &mut child,
            )
            .await
        ...
        let cases = [
            ...
            format!("GET {valid} HTTP/1.1\r\nHost: {address}\r\nHost: {address}\r\n\r\n"),
            format!("GET {valid} HTTP/1.1\r\n\r\n"),
            format!("POST {valid} HTTP/1.1\r\nHost: {address}\r\n\r\n"),
            format!("GET /callback?state=wrong&code={code} HTTP/1.1\r\nHost: {address}\r\n\r\n"),
            format!("GET {valid}&state={state} HTTP/1.1\r\nHost: {address}\r\n\r\n"),
            format!("GET {valid}&code={code} HTTP/1.1\r\nHost: {address}\r\n\r\n"),
            format!("GET {valid}&error=failed HTTP/1.1\r\nHost: {address}\r\n\r\n"),
            format!("GET {valid}&extra=1 HTTP/1.1\r\nHost: {address}\r\n\r\n"),
            format!("GET /callback?state={state} HTTP/1.1\r\nHost: {address}\r\n\r\n"),
            format!("GET /other?state={state}&code={code} HTTP/1.1\r\nHost: {address}\r\n\r\n"),
        ];
        for request in cases {
            ...
            assert!(
                !wait.is_finished(),
                "rejected request cannot hand a code to exchange"
            );
        }
        let mut socket = tokio::net::TcpStream::connect(address).await.unwrap();
        socket
            .write_all(format!("GET {valid} HTTP/1.1\r\nHost: {address}\r\n\r\n").as_bytes())
            .await
            .unwrap();
        let mut bytes = Vec::new();
        tokio::time::timeout(
