...
use access_oidc::{OidcError, Provider, ProviderConfig};
...
async fn real_provider_flows() -> anyhow::Result<()> {
    tokio::time::timeout(Duration::from_secs(120), async {
        flow(&std::env::var("ACCESS_TEST_KEYCLOAK_ISSUER")?, None).await?;
        flow(
            &std::env::var("ACCESS_TEST_HYDRA_ISSUER")?,
            Some(&std::env::var("ACCESS_TEST_HYDRA_ADMIN")?),
        )
        .await?;
        assert!(
