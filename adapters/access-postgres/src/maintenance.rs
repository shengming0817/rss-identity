...
use access_core::account::{LoginKey, Password, PasswordKdf, SecurityAction};
                ...
                crate::transaction::connection(tx, move |c| {
                    Box::pin(async move {
                        let initialized: bool = sqlx::query_scalar(
                            "SELECT bootstrap_tenant IS NOT NULL FROM access_authority.deployment FOR UPDATE",
                        )
                        .fetch_one(&mut *c)
                        .await?;
                        if initialized {
                        ...
                        sqlx::query("UPDATE access_authority.deployment SET bootstrap_tenant=$1::uuid")
            ...
            Box::pin(async move {
                crate::transaction::connection(tx, move |c| {
                    Box::pin(async move {
                        ...
                        sqlx::query(
                            "UPDATE access_authority.accounts SET password_hash=$3,auth_epoch=$4,credential_version=$5
