...
当前方向：2026-09-19，#2435 可嵌入认证组件及 #2447 可选可信部门事实。此版本替换中央认证服务产品形态；历史需求与中央模式验收可在 fa7019922162158704cc47c6ac7ad36a67c8ae5a 查询，不能作为当前能力证明。
...
| ACC-13 可选部门事实 | provider 同时配置独立 claim 和 1–300 秒期限；仅从已验签上游采集受控单值编码，显式 null 表示无部门，缺失保持未知。可信读取绑定实例/tenant/principal/provider/config，refresh 不延期，配置/账户/session 撤销须生效；资源权限归宿主，不含部门目录、审批或 MDM/Web 接入。 |

## 不兼容策略

未部署产品直接替换：schema v10 只支持全新安装，联合认证事实仅接受 format v2，旧 schema/config/API 不提供升级、别名、dual-read 或 feature 回退。旧数据库、凭据和已存验收记录不被自动修改或重标为成功。OIDC 为可选能力，本地认证不要求联邦密钥或中央控制面。
