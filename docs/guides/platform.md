...
CLI 对 `invalid_platform_request`、`platform_conflict`、`tenant_limit_reached`、`rate_limited` 等闭集服务端拒绝显示具体 code，退出码仍为 11；未知 code 不透传原文。断网、超时、未知服务响应不自动重放写入。查不到回执不能证明服务器原事务已经结束或未提交。跨租户增加管理员的审计与账户、凭据和成员在同一个 PG 事务中；只有确认提交才返回结果。
...
IdP 凭据加密 keyring 位于数据库外，每把密钥是私有文件中的 64 位十六进制。先部署包含新 active key 与旧解密钥的配置并重启，再在独立 owner 任务中反复运行 `identity-migrate --rekey --config /run/config/migration.json`，直至 `rewritten=0`。每次最多选择、锁定并重写 100 个旧钥凭据，同一事务结算；已使用 active key 的行不进入锁定集合。候选按 tenant/provider 稳定排序，成功行退出旧钥集合，重复执行自然推进而不增加持久化游标。整个连接、事务、提交和关闭共用 60 秒截止时间，超时丢弃专用连接；已确认提交不会因关闭失败而改称未知。结果未知时重新核验/执行，已使用 active key 的行不会再次改写。只有确认当前数据全部可解密且无旧钥引用后才能退出旧钥；备份仍需保留其切点对应的钥。启动会逐条验证密文认证；旧钥缺失、同名密钥材料错误或密文损坏使启动核验或解密失败，不静默接受损坏数据。

`make test-platform` 验证真实 CLI、HTTPS、后端及 PG；`make test-pg` 包含平台角色、事务和 CLI SSO 码接缝。完整生产双租户 SSO 验收由 #2342 持有，此指南不代表 T3 已通过。

## 中央网页（#2368）

部署向平台管理员提供系统域的 `/tenants/{system-domain}/login` 地址；中央网页按当前会话返回的
显式平台角色显示入口，再通过 `GET /api/v1/platform` 核实系统域。网页不提供匿名系统域发现，
不把普通租户 administrator 标志提升为平台权限。

平台页列出租户并开通新租户，网页生成 tenant/principal/operation UUID，输入首个管理员登录名和
口令。201 表示已提交且可登录，202 表示已提交待激活；失败和未知结果保留原 operation 查询定位。
404 未观察到回执不证明原事务未提交；刷新、重新登录和查询均不重发创建请求。
口令不进入路由或持久存储，确认回执不因后续列表读取失败而改称未知。

成功结果给出业务租户登录地址，平台会话不自动变成该租户会话。网页不消费跨租户增加管理员或
平台角色授予/撤销接口；这些既有后端能力不因页面导航存在而扩大。双仓版本与实际网页接缝结果
记录在 #2368 的关联 PR；本指南本身不构成验收完成声明。
