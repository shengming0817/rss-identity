...
可再生入口和要求见[候选构建](../../images.md)：在隔离 Linux Docker 主机运行 `make test-reference`；验证既有记录使用同一候选的 `reference_seams.py --candidate ... --record ... --verify-record`。本地可再生候选目录为 `rss-external-check/identity-1030-89b8769`，唯一源码和证据均已提交，不依赖该目录保存。
