...
部署 owner 决定恢复点及可接受数据损失，独立保管配置、证书、密码和所需钥集合。工具保持实例与 tenant fence，不判断历史快照是否包含最新撤销；未核对完整安全状态前保持关闭。每条命令都传候选目录、私有部署目录和明确 project，以下以 `op` 表示这个固定前缀。

```sh
op() { python3 /artifacts/operate.py --candidate /artifacts --deployment /private/rendered --project identity-main "$@"; }
op close
...
```sh
python3 /artifacts/operate.py --candidate /artifacts --deployment /private/restored --project identity-restored restore /private/backups/cut-001.dump
python3 /artifacts/operate.py --candidate /artifacts --deployment /private/restored --project identity-restored verify
