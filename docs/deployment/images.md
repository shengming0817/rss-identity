...
省略 `REFERENCE_TARGETS` 时，只生成 `measured` 基线，不能标记生产目标验收通过。owner 根据基线确认支持规模、SLO、RPO/RTO 后，提供新的私有 JSON：`subject` 为基线固定候选对象，`baselineSha256` 为完整 measured 基线文件摘要；`approvalReference` 为本次 T3 PR 的批准评论链接（固定组织/项目，仅允许 `discussionId` 参数），`limits` 明确包含 loginP95Ms、loginBurstRequestsPerSecond、failedAttemptP95Ms、failedAttemptRequestsPerSecond、accountEventCommitP95Ms、accountEventCommitsPerSecond、sessionP95Ms、sessionRequestsPerSecond、unexpectedErrors、restoreSeconds、lostSecurityChanges、expiredAttemptsRemoved。当前基线记录实际数据表行数；登录是 5 个正确凭据的短突发，失败尝试含真实限流，权威会话分别以 1/4/16 并发持续 30 秒，不能将短突发速率宣称为绕过既有预算的持续登录能力。耗时、错误和丢失数量为上限；吞吐和清理数量为下限。再次调用同一入口并设置 `REFERENCE_TARGETS=/absolute/approved-targets.json REFERENCE_BASELINE=/absolute/measured-result.json`，回读并验证原基线、候选、场景和清理完整，再使用全新输出和数据卷正式复测。审批评论是可审计的人类决定引用，运行器不把任意 URL 当作批准内容证明。
