...
参考宿主提供固定 300 秒 MFA 示范资源，直接消费当前权威 session 的可信 assurance，并在网关挂载。它不改变普通管理、本地应急或核心认证策略。Rust 构造器直接替换，旧 v3 配置拒绝；schema v9 和 HTTP v2 保持现有身份。真实浏览器、恢复及容量证据归 #2366。
