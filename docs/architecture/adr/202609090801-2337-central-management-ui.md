...
| Authority 公共 provider CRUD / 任意测试 closure | Federation 管理门面；Authority 包内持久化 | 配置批准、CAS、前后权限重验；删除公共旁路 |
...
每次事务内重验 AuthenticatedSession，包括存储 authority、cookie digest、tenant、期限、账户/成员 epoch 与当前管理资格。早期管理员快照检查仅用于拒绝不必要的 KDF/静态配置工作，不授予操作权限。目标和 actor 的 tenant 必须一致。
...
IdP create/update 只验证完整部署许可，不要求当前秘密已加载；测试/登录才要求秘密可用。只读/停用依赖 PG 与有效管理员会话。Federation 注入 UpstreamOidc；approve_configuration 和实际网络工作分开。测试前后核对权限及配置版本，只有审计结算确认后才返回报告，安全诊断限 ProviderStage/ProviderReason。当前状态及本次测试足以完成 I07，不新增 last-test 表。
