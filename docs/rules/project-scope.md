...
宿主持有管理角色、防锁死、资源授权、配置、实例标识、租户 admission、数据库角色与 fenced PgRuntime；组件强制实例/租户/主体/epoch/会话绑定并在事务内调用宿主策略。组、可选部门和 assurance 仅提供可信认证事实，不生成资源权限。MDM/ZT 持有设备 authority、attestation、posture 和业务授权。

仅支持配置 v4、HTTP v2 和 fresh schema v10，无旧配置、旧库升级、v1 alias、legacy feature 或中央 downstream/client/Hydra 模式。重新部署及真实产品迁移须由其独立任务闭合。
