...
const state = fs.existsSync(input.privateFile)
  ? JSON.parse(fs.readFileSync(input.privateFile, "utf8"))
  : { contexts: {}, data: {}, secrets: [] };
...
async function open(role, t = tenant, fresh = false) {
  ...
  const context = await browser.newContext({
    baseURL: origin,
    storageState: fresh ? undefined : state.contexts[role],
    locale: "en-US",
  });
  const page = await context.newPage();
  page.setDefaultTimeout(20000);
  page.setDefaultNavigationTimeout(30000);
  const value = { role, t, context, page };
  active.push(value);
  await page.goto(`${origin}/tenants/${t}/login`);
  return value;
}
async function store(c) {
  state.contexts[c.role] = await c.context.storageState();
  for (const cookie of state.contexts[c.role].cookies) remember(cookie.value);
  save();
}
async function read(c, path) {
  const response = await c.context.request.get(origin + path);
  let body = null;
  try {
...
async function write(c, method, suffix, body) {
  ...
  check(session.status === 200, "writer-session");
  remember(session.body.csrfToken);
  return request(c, method, `/api/v2/tenants/${c.t}/${suffix}`, body, {
    "x-csrf-token": session.body.csrfToken,
  });
}
async function sessions(c) {
  await c.page.goto(`${origin}/tenants/${c.t}/sessions`);
  await c.page.locator("h1").waitFor();
}
async function login(c, name, password) {
  diagnostic = "local-login-form";
  await c.page.goto(`${origin}/tenants/${c.t}/login`);
  await c.page.locator("#login-name").fill(name);
  await c.page.locator("#login-password").fill(password);
  const response = c.page.waitForResponse(
    (r) =>
      r.request().method() === "POST" &&
      new URL(r.url()).pathname === `/api/v2/tenants/${c.t}/login`,
  );
  await c.page
    ...
    .click();
  check((await response).status() === 200, "ui-local-login");
...
async function admin(t = tenant) {
  ...
  if ((await current(c)).status !== 200)
    await login(
      c,
      "operator",
      t === tenant
        ? (data.recoveredAdminPassword ?? input.adminPassword)
        : input.adminPassword,
    );
  await sessions(c);
  const session = await current(c);
  if (Date.now() / 1000 - session.body.session.authTime >= 270) {
    ...
    const response = c.page.waitForResponse(
      (r) =>
        r.url().endsWith("/session/reauthenticate") &&
        r.request().method() === "POST",
    );
    await c.page
      .locator("#reauth-password")
      ...
      .locator("button")
      .click();
    check((await response).status() === 200, "admin-reauthentication");
    await c.page.reload();
    await store(c);
  }
  return c;
...
async function createAccount(c, name, password, expected = 201) {
  await c.page.goto(`${origin}/tenants/${c.t}/accounts`);
  await c.page.locator("#account-login").fill(name);
  await c.page.locator("#account-password").fill(password);
  const response = c.page.waitForResponse(
    (r) => r.url().endsWith("/accounts") && r.request().method() === "POST",
  );
  await c.page
  ...
  check(r.status() === expected, "ui-create-account");
  if (expected === 201) {
...
async function provider(c) {
  const r = await read(c, `/api/v2/tenants/${c.t}/providers`);
  check(r.status === 200 && r.body.providers.length === 1, "provider-list");
  return r.body.providers[0];
}
async function createProvider(c) {
  await c.page.goto(`${origin}/tenants/${c.t}/providers`);
  await c.page.locator("#issuer").fill(input.issuer);
  await c.page.locator("#client-id").fill("reference");
  await c.page.locator("#client-secret").fill(input.clientSecret);
  await c.page.locator("#ca-pem").fill(input.idpCa);
  await c.page.locator("#scopes").fill("openid");
  await c.page.locator("#email-claim").fill("");
  await c.page.locator("#groups-claim").fill("");
  await c.page.locator("#jit").check();
  const response = c.page.waitForResponse(
    (r) => r.url().endsWith("/providers") && r.request().method() === "POST",
  );
  await c.page
    .locator("#issuer")
    .locator("xpath=ancestor::form")
    ...
    .click();
  const r = await response;
  check(r.status() === 201, "ui-provider-create");
  const p = await r.json();
  const enabled = await write(c, "POST", `providers/${p.id}/enabled`, {
    expectedVersion: p.version,
    enabled: true,
  });
  check(enabled.status === 200, "provider-enable");
  data.providers ??= {};
  data.providers[c.t] = p.id;
  await store(c);
}
async function expectOldCookie(c, cookies, t = tenant) {
  ...
  const r = await temp.request.get(
    `${origin}/api/identity-host/v1/tenants/${t}/context`,
  );
  check(r.status() === 401, "old-cookie-rejected");
  await temp.close();
}
function totp() {
  ...
  const mac = crypto
    .createHmac("sha1", input.otpSecret)
    .update(counter)
    .digest();
  const offset = mac[19] & 15;
  return remember(
    String((mac.readUInt32BE(offset) & 0x7fffffff) % 1000000).padStart(6, "0"),
  );
}
async function keycloak(
  ...
  await c.page
    .locator("#password")
    .fill(badPassword ? "wrong-private-password" : input.idpPassword);
  await c.page.locator("#kc-login").click();
  if (badPassword) {
    await c.page.locator("#password").waitFor();
    check(
      !(await c.context.cookies(origin)).some(
        (x) => x.name === "__Host-identity-session",
      ),
      "wrong-idp-password-no-session",
    );
    await c.page.locator("#password").fill(input.idpPassword);
    await c.page.locator("#kc-login").click();
  }
  if (otp) {
    ...
    if (badOtp) {
      ...
      await c.page.locator("#otp").fill(wrong);
      await c.page.locator("#kc-login").click();
      await c.page.locator("#otp").waitFor();
    }
  ...
  if (success) {
    await c.page.waitForURL(`**/tenants/${c.t}/sessions`);
    check((await current(c)).status === 200, "oidc-session");
  } else
    check(
      (await read(c, `/api/identity-host/v1/tenants/${c.t}/mfa-example`))
        .status !== 200,
      "failed-mfa-not-authorized",
    );
  await store(c);
}
async function sso(role, user, t = tenant, options = {}) {
  const c = await open(role, t, true);
  await c.page
    .getByRole("button", { name: /Organization SSO|组织 SSO/ })
    .click();
  await keycloak(c, user, options);
  return c;
}
async function stepUp(c, user = "bob", options = {}) {
  ...
  await c.page
    ...
    .click();
  await keycloak(c, user, { otp: true, ...options });
}
async function local() {
  ...
  check(
    cookie &&
      ...
      cookie.sameSite === "Lax" &&
      cookie.path === "/" &&
      cookie.domain === "identity.example.test",
    "browser-cookie-attributes",
  );
  check(
    (await read(a, `/api/identity-host/v1/tenants/${other}/context`)).status ===
      401,
    "tenant-isolation",
  );
  ...
  for (const name of ["local", "linked", "disabled", "inactive", "limited"])
    await createAccount(a, name, input.userPassword);
  const c = await open("local", tenant, true);
  await login(c, "local", input.userPassword);
  check(
    (await read(c, `/api/v2/tenants/${tenant}/accounts`)).status === 403,
    "nonmanager-server-denial",
  );
  const before = await current(c);
  const old = await c.context.cookies();
  const bad = await c.context.request.post(
    `${origin}/api/v2/tenants/${tenant}/session/refresh`,
    {
      headers: {
        origin,
        "x-identity-request": "1",
  ...
  check(bad.status() === 403, "csrf-rejection");
  const evil = await c.context.request.post(
    `${origin}/api/v2/tenants/${tenant}/session/refresh`,
    {
      headers: {
        origin: "https://untrusted.example.test",
        "x-identity-request": "1",
        "x-csrf-token": before.body.csrfToken,
      },
    },
  );
  check(evil.status() === 403, "origin-rejection");
  const refreshed = await write(c, "POST", "session/refresh", {});
  check(
    refreshed.status === 200 &&
      refreshed.body.session.authTime === before.body.session.authTime &&
      refreshed.body.session.absoluteExpiresAt ===
        before.body.session.absoluteExpiresAt,
    "refresh-lifetimes",
  );
  await expectOldCookie(c, old);
  await sessions(c);
  await c.page.locator("#current-password").fill(input.userPassword);
  await c.page.locator("#new-password").fill(input.nextPassword);
  const changed = c.page.waitForResponse(
    (r) =>
      r.url().endsWith("/account/password") && r.request().method() === "POST",
  );
  await c.page
    ...
    .locator("button[type=submit]")
    .click();
  check((await changed).status() === 200, "ui-change-password");
  await c.page.waitForURL(`**/tenants/${tenant}/login`);
  await login(c, "local", input.nextPassword);
  const currentCookie = await c.context.cookies();
  await c.page
  ...
  const blocked = await request(
    ...
    "POST",
    `/api/v2/tenants/${tenant}/login`,
    { login: "limited", password: input.userPassword },
  );
  check(blocked.status === 429, "correct-password-obeys-budget");
  await store(a);
  await store(b);
  return {
...
async function oidc() {
  const a = await admin();
  const b = await admin(other);
  await createProvider(a);
  await createProvider(b);
  const alice = await sso("alice", "alice", tenant, { badPassword: true });
  const one = await current(alice);
  data.alicePrincipal = one.body.identity.principalId;
  const aliceOther = await sso("alice-other", "alice", other);
  check(
    (await current(aliceOther)).body.identity.principalId !==
      data.alicePrincipal,
    "jit-tenant-identity-isolation",
  );
  const linked = await open("linked", tenant, true);
  await login(linked, "linked", input.userPassword);
  await linked.page
    .locator("#link-provider")
    .selectOption(data.providers[tenant]);
  await linked.page.locator("#link-password").fill(input.userPassword);
  await linked.page
    .locator("#link-password")
    .locator("xpath=ancestor::form")
    .locator("button")
    .click();
  await keycloak(linked, "bob");
  check(
    (await current(linked)).body.identity.principalId === data.accounts.linked,
    "explicit-link-subject",
  );
  check(data.accounts.linked !== data.alicePrincipal, "no-email-auto-link");
  const weak = await read(
    linked,
    `/api/identity-host/v1/tenants/${tenant}/mfa-example`,
  );
  check(weak.status === 403, "ordinary-login-is-not-mfa");
  await store(linked);
  ...
  await bound.page
    .getByRole("button", { name: /Organization SSO|组织 SSO/ })
    .click();
  await bound.page.locator("#username").fill("alice");
  await bound.page.locator("#password").fill(input.idpPassword);
  await bound.page.locator("#kc-login").click();
  for (let i = 0; !callback && i < 100; i++)
...
async function mfa() {
  ...
  await c.page.route(
    ...
    async (route) => {
      ...
      if (!downgraded && u.searchParams.get("acr_values") === "2") {
        u.searchParams.set("acr_values", "1");
        downgraded = true;
  ...
  check(
    resource.status === 200 && resource.body.authentication.acr === "mfa",
    "fresh-mfa-resource",
  );
  check(resource.body.principalId === data.accounts.linked, "mfa-subject");
  const authTime = resource.body.authentication.authTime;
  const refresh = await write(c, "POST", "session/refresh", {});
  check(refresh.status === 200, "mfa-refresh");
  resource = await read(
    c,
    `/api/identity-host/v1/tenants/${tenant}/mfa-example`,
  );
  check(
    resource.body.authentication.authTime === authTime,
  ...
  const disabled = await write(a, "POST", `providers/${p.id}/enabled`, {
    expectedVersion: p.version,
    enabled: false,
  });
  check(disabled.status === 200, "provider-disabled");
  check((await current(c)).status === 401, "provider-disable-revokes-session");
  const enabled = await write(a, "POST", `providers/${p.id}/enabled`, {
    expectedVersion: disabled.body.version,
    enabled: true,
  });
  check(enabled.status === 200, "provider-reenabled");
  check(
    (await current(c)).status === 401,
...
async function responseLoss() {
  const a = await admin();
  await a.page.goto(`${origin}/tenants/${tenant}/accounts`);
  let requests = 0;
  await a.page.route(
    `${origin}/api/v2/tenants/${tenant}/accounts`,
    async (route) => {
      ...
      check(reply.status() === 201, "lost-response-actually-committed");
      await route.abort("connectionreset");
    },
  );
  await a.page.locator("#account-login").fill("uncertain");
  await a.page.locator("#account-password").fill(input.userPassword);
  await a.page
    .locator("#account-password")
    .locator("xpath=ancestor::form")
    .locator("button[type=submit]")
  ...
  await store(a);
  return { singleWrite: true, noFalseUiSuccess: true };
}
async function providerReady() {
  ...
  for (let i = 0; i < 30; i++) {
    const p = await provider(a);
    const r = await write(a, "POST", `providers/${p.id}/test`, {});
    if (r.status === 200 && r.body.passed === true) {
      okay = true;
      break;
    }
    await new Promise((r) => setTimeout(r, 1000));
  }
  check(okay, "provider-ready");
  await store(a);
  return { providerReachable: true };
}
async function beginState() {
  const c = await open("pending-state", tenant, true);
  const r = await request(
    c,
    "POST",
    `/api/v2/tenants/${tenant}/oidc/${data.providers[tenant]}/login`,
    { returnTarget: "resume" },
  );
...
async function recoveryState() {
  ...
  for (const [name, field] of [
    ...
    const r = await write(
      a,
      "POST",
      `accounts/${data.accounts[name]}/${field}`,
      { enabled: false },
    );
    check(r.status === 200, "recovery-negative-account-state");
  }
  data.preRecoveryAdminCookies = await a.context.cookies();
  await store(a);
...
let observations;
try {
  diagnostic = "browser-launch";
  browser = await chromium.launch({ headless: true });
  switch (input.phase) {
    case "local":
      observations = await local();
      break;
    case "oidc":
      observations = await oidc();
      break;
    case "mfa":
      observations = await mfa();
      break;
    case "rollback": {
      const a = await admin();
    ...
    case "response-loss":
      observations = await responseLoss();
      break;
    case "idp-down": {
      const local = await open("emergency-local", other, true);
      await login(local, "operator", input.adminPassword);
      const a = await admin();
      const p = await provider(a);
      const result = await write(a, "POST", `providers/${p.id}/test`, {});
      check(
    ...
    case "provider-ready":
      observations = await providerReady();
      break;
    case "pg-down": {
      const a = await open("admin");
      check(
        (await read(a, `/api/identity-host/v1/tenants/${tenant}/context`))
          .status === 503,
        "pg-resource-denied",
      );
      const anon = await open("pg-login", other, true);
      const r = await request(anon, "POST", `/api/v2/tenants/${other}/login`, {
        login: "operator",
        password: input.adminPassword,
      });
      check(r.status === 503, "pg-login-denied");
      observations = { authorityUnavailable: true };
      break;
    }
    case "pg-ready": {
      const a = await admin();
      ...
      await store(a);
      observations = { storageRecovered: true };
      break;
    }
    case "begin-old-state":
      observations = await beginState();
      break;
    case "reject-old-state": {
      const c = await open("pending-state");
      await c.page.goto(data.pendingState);
      await keycloak(c, "alice", { success: false });
      check((await current(c)).status === 401, "old-state-no-session");
      observations = { oldStateRejected: true };
      break;
    }
    case "fresh-sso": {
      const c = await sso("fresh-sso", "alice");
      await store(c);
      observations = { freshSso: true };
      break;
    }
    ...
    case "recovery-state":
      observations = await recoveryState();
      break;
    case "restored": {
      const c = await open("restored-admin", tenant, true);
      await expectOldCookie(c, data.preRecoveryAdminCookies);
      const old = await request(c, "POST", `/api/v2/tenants/${tenant}/login`, {
        login: "operator",
        password: input.adminPassword,
      });
      check(old.status === 401, "restored-old-password");
      data.recoveredAdminPassword = input.recoveredAdminPassword;
      const a = await admin();
      check((await current(a)).status === 200, "restored-new-password");
      for (const name of ["disabled", "inactive"]) {
        const r = await request(c, "POST", `/api/v2/tenants/${tenant}/login`, {
          login: name,
          password: input.userPassword,
        });
      ...
      observations = {
        ...
        localAndOidcRecovered: true,
      };
      break;
    }
    case "capacity":
      observations = await capacity();
      break;
    default:
  ...
  if (network)
    diagnostic += "-" + network[1].toLowerCase().replaceAll("_", "-");
  else if (failure?.name === "TimeoutError") diagnostic += "-timeout";
  save();
  fs.writeFileSync(
    input.privateFile + ".failure",
    JSON.stringify({ phase: input.phase, diagnostic }),
    { mode: 0o600 },
  );
  console.log(
