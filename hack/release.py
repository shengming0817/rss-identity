...
def validate_ui(source,dist):
 ...
 canonical='https://dev.azure.com/shengming0923/rss/_git/rss-web'
 if remote!=canonical:raise ValueError('unapproved UI repository identity')
 subprocess.run(['node',str(source/'scripts/check-identity-app-build.mjs'),str(dist)],cwd=source,check=True)
...
def build(out,ui_source,ui_dist):
 ...
 with tempfile.TemporaryDirectory(prefix='identity-candidate-') as temp:
  ...
  result={'format_version':1,'config_version':3,'revision':revision,'version':tomllib.loads((ROOT/'Cargo.toml').read_text())['workspace']['package']['version'],'platform':'linux/amd64','cargo_lock_sha256':sha(ROOT/'Cargo.lock'),'rust':tomllib.loads((ROOT/'rust-toolchain.toml').read_text())['toolchain']['channel'],'ui':ui,'providers':images,'migration_sha256':sha(ROOT/'crates/identity-postgres/migrations/0001_authority.sql'),'images':{},'archives':{}}
  ...
  for target in ['server','operator','gateway']:
   ...
   if target=='server':
    ...
    subprocess.run(['docker','run','--rm','--network','none','--platform','linux/amd64','--entrypoint','identity-admin',name,'--version'],check=True)
   if target=='gateway':
