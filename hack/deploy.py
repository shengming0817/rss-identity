...
''')
    ...
    def service(image,names,command=None):
        ...
        return v
    images=candidate['images'];services={}
    services['postgres']=service(candidate['providers']['postgres'],['owner-password','postgres-cert','postgres-key','00-roles.sql','pg_hba.conf'],['postgres','-c','ssl=on','-c','ssl_cert_file='+pgcert,'-c','ssl_key_file='+pgkey,'-c','hba_file=/run/config/pg_hba.conf'])
    ...
    services['migrate']=service(images['operator'],['migration.json','owner-password','database-ca']+key_files+(['keyring.json'] if key_files else []),['--config','/run/config/migration.json'])
    services['migrate']['profiles']=['operator'];services['migrate']['depends_on']={'postgres':{'condition':'service_healthy'}}
    services['maintenance']=service(images['operator'],['maintenance.json','maintenance-password','database-ca'])
    services['maintenance'].update(profiles=['operator'],entrypoint=['identity-admin','/run/config/maintenance.json'])
    services['identity']=service(images['server'],runtime_files+['runtime.json'],['--config','/run/config/runtime.json'])
