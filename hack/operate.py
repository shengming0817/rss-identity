...
def check_backup(path,backend_version=None,config=None):
    ...
    if type(record['schema']) is not int or record['schema']!=11:raise Rejection('backup-schema')
...
def execute(args,backend_version,compose,directory,record):
    ...
    if args.command=='install':
        ...
        record={'schema':11,'instanceId':config['instanceId'],'storage':config['storage'],'backendVersion':backend_version,'project':args.project,'sha256':sha(path)}
