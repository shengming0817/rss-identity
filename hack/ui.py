...
class Gateway(http.server.SimpleHTTPRequestHandler):
    ...
    def end_headers(self):
        ...
        super().end_headers()
    def do_GET(self):
        if self.path == '/api/identity-host/v1/config.json':
            data=json.dumps({'canonicalOrigin':self.server.origin,'oidcEnabled':True}).encode()
    ...
    def proxy(self):
        ...
        try:
            headers={k:v for k,v in self.headers.items() if k.lower() not in ('connection','content-length','forwarded','x-forwarded-for','x-real-ip')}
        ...
        with tempfile.TemporaryDirectory(prefix='identity-ui-t2-') as directory:
            ...
            with contextlib.ExitStack() as stack:
                try:
                    ...
                    env = {'NODE_EXTRA_CA_CERTS':str(cert), 'IDENTITY_TEST_PG_PORT': str(ports[5432]),
                    ...
                    providers.cargo('rss-identity-app', 'ui_host', env)
