...
def run():
    subprocess.run(['cargo','build','--locked','-p','rss-identity-platform'],cwd=providers.ROOT,check=True)
    ...
    with tempfile.TemporaryDirectory(prefix='identity-platform-t2-') as tmp, providers.postgres() as (_,ports):
        ...
        commands=[
            ['openssl','req','-x509','-newkey','rsa:2048','-nodes','-days','1','-subj','/CN=CLI Fixture CA','-keyout',str(root/'ca-key.pem'),'-out',str(root/'ca.pem')],
        ...
        class Proxy(http.server.BaseHTTPRequestHandler):
            ...
            def forward(self):
                ...
                try:
                    ...
                    with (root/'http-events.jsonl').open('a') as log:log.write(json.dumps({'method':self.command,'path':self.path,'status':response.status})+'\n')
                    drop=root/'drop-next'
