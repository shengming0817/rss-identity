...
/// Non-nil Access PrincipalId; never inferred from browser claims.
...
/// Non-nil Access SessionId; never inferred from browser claims.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// Storage snapshot only; deliberately not named or serializable as VerifiedAccessContext.
#[derive(Debug, Clone)]
...
impl SessionSnapshot {
    ...
    /// Internal Access principal; not a public cross-product subject.
    pub fn principal(&self) -> PrincipalId {
        self.principal
    }
    /// Access authentication session identity.
