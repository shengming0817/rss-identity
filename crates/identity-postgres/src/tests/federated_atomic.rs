...
async fn session_security_is_subject_bound_and_revocable() -> anyhow::Result<()> {
    ...
    let snapshot = s
        ...
        .await?;
    assert_eq!(snapshot.session_id, local.view().id);
    assert_eq!(
        snapshot.assurance.auth_time().unwrap(),
        local.view().auth_time
    );
    assert_eq!(snapshot.assurance.acr(), Acr::Unspecified);
    assert_eq!(snapshot.assurance.amr(), [Amr::Pwd]);
...
async fn federation_configuration_authorization_and_versions() -> anyhow::Result<()> {
    ...
    *upstream.hook.lock().unwrap() = Some(Box::new(|| Box::pin(std::future::pending())));
    ...
    f.store
        .create_local_account(actor(&f).await?, login("member"), password(), deadline())
        .await?;
    let member = f
        .store
        .verify_password(
    ...
    assert!(
        s.create_provider(
            crate::session_actor(&f.store, member).await?,
            settings(),
            rss_identity_core::federation::ProviderCredentials::new("fixture-secret".into(), None)
                .unwrap(),
            deadline()
...
async fn federation_jit_isolated_subjects_and_membership() -> anyhow::Result<()> {
    ...
    sqlx::query("INSERT INTO identity_authority.guard(tenant_id) VALUES($1::uuid)")
        ...
        .execute(&f.owner)
        .await?;
    sqlx::query("INSERT INTO identity_authority.accounts(tenant_id,principal_id,enabled,auth_epoch) SELECT $1::uuid,principal_id,enabled,auth_epoch FROM identity_authority.accounts WHERE tenant_id=$2::uuid AND principal_id=$3").bind(B).bind(A).bind(f.key.principal.as_uuid()).execute(&f.owner).await?;
    ...
    let provider_b = s
        .create_provider(
            crate::session_actor(&f.store, admin_b).await?,
            settings(),
            rss_identity_core::federation::ProviderCredentials::new("fixture-secret".into(), None)
                .unwrap(),
            deadline(),
...
async fn federation_concurrent_jit_rls_and_schema_drift() -> anyhow::Result<()> {
    ...
    *upstream.gate.lock().unwrap() = Some(gate.clone());
    ...
    let guards: i64 = sqlx::query_scalar("SELECT count(*) FROM identity_authority.guard")
        ...
        .await?;
    assert_eq!(guards, 2);
    sqlx::raw_sql("GRANT DELETE ON identity_authority.providers TO identity_runtime")
        .execute(&f.owner)
        .await?;
    assert!(matches!(
    ...
    sqlx::raw_sql("REVOKE DELETE ON identity_authority.providers FROM identity_runtime; ALTER TABLE identity_authority.oidc_transactions DROP CONSTRAINT oidc_transactions_state_hash_check").execute(&f.owner).await?;
    assert!(matches!(
        f.probe(AuthorityProfile::Runtime).await,
        Err(AuthorityError::StorageIncompatible(
