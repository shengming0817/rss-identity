...
async fn maintenance_permissions_and_schema_are_exact() -> anyhow::Result<()> {
    ...
    sqlx::raw_sql("UPDATE identity_authority.schema_version SET version=9; ALTER TABLE identity_authority.schema_version ADD CHECK(version=9)").execute(&f.owner).await?;
    assert!(f.probe(AuthorityProfile::Runtime).await.is_ok());
