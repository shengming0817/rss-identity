WITH protected AS (
 ...
 WHERE n.nspname='identity_authority' AND c.relname IN ('guard','accounts','memberships','attempts')
), policies AS (
 ...
 SELECT
 (SELECT count(*)=1 AND bool_and(version=2) FROM identity_authority.schema_version) AS version_ok,
